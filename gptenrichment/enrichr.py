...
def get_enrichment_results(genes, gene_set_library):
    ENRICHR_URL = get_config()["ENRICHR_API"]+'/addList'
    genes_str = '\n'.join(genes)
    ...
    ENRICHR_URL = get_config()["ENRICHR_API"]+'/enrich'
    ...
    df = pd.DataFrame(data[gene_set_library], columns=[
        "Rank", "Term", "P-value", "Z-score", "Combined score", "Genes", "Adjusted p-value", "v1", "v2"
    ])
    return df.iloc[:,0:6]